...
class TestThreatLevel(unittest.TestCase):
    def test_get_threat_level(self):
        self.assertEqual(
            ThreatLevel("failed_attempt").get_threat_level(), ThreatLevel.HIGH
        )
        self.assertEqual(
            ThreatLevel("invalid_user").get_threat_level(), ThreatLevel.MEDIUM
        )
        self.assertEqual(
            ThreatLevel("session_opened").get_threat_level(), ThreatLevel.NO_THREAT
        )
        self.assertEqual(
            ThreatLevel("unknown_type").get_threat_level(), ThreatLevel.UNKNOWN
        )
...
class TestLogType(unittest.TestCase):
    def test_get_type(self):
        ...
        self.assertEqual(
            LogType.get_type("Accepted password"), LogType.SUCCESSFUL_LOGIN
        )
...
class TestLogEntry(unittest.TestCase):
    def test_log_entry(self):
        date = datetime.now()
        log_entry = LogEntry(
            date, "localhost", "sshd", "Failed password", "192.168.1.1"
        )
...
class TestAuthLogAnalyzer(unittest.TestCase):
    def test_identify_log_type(self):
        ...
        self.assertEqual(
            analyzer._identify_log_type("Failed password"), LogType.FAILED_ATTEMPT
        )

    def test_hash_line(self):
        analyzer = AuthLogAnalyzer("dummy.log")
        self.assertEqual(
            analyzer._hash_line("test line"),
            hashlib.sha256("test line".encode()).hexdigest(),
        )
...
class TestPerIpCounter(unittest.TestCase):
    def test_count_requests_per_ip(self):
        log_data = [
            LogEntry(
                datetime.now(), "localhost", "sshd", "Failed password", "192.168.1.1"
            ).to_dict(),
            LogEntry(
                datetime.now(), "localhost", "sshd", "Invalid user", "192.168.1.1"
            ).to_dict(),
        ]
